...
class SSW:
    ...
    def readHeader(self, filename, nevt=0):
        ...
        if size == 8:  # mcnp 6
            ...
            data = fortranRead(self.file)
            (self.kods, self.vers, self.lods, self.idtms, self.aids, self.knods) = struct.unpack("=8s5s28s18s80si",
                                                                                                 data)
            ...
            (self.kods, self.vers, self.lods, self.idtms, self.probs, self.aids, self.knods) = struct.unpack(
                "=8s5s28s19s19s80si", data)  # length=160
            self.vers = self.vers.decode().strip()
            self.probs = self.probs.decode().strip()

        elif size == 167:  # like in the Tibor's file with 2.7.0
            tmp = float(0)
            (self.kods, self.vers, self.lods, self.idtms, self.probs, self.aids, self.knods, tmp) = struct.unpack(
                "=8s5s28s19s19s80sif", data)  # length=160
            self.vers = self.vers.decode().strip()
            self.probs = self.probs.decode().strip()
        else:
            self.unsupported()
        ...
        if self.vers in self.supported_mcnp6_versions:
            ...
            #print("probs", np1, nrss, self.nrcd, njsw, niss)
        elif size == 20:
